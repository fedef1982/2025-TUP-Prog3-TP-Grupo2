...
import { createDonation } from '@/app/lib/actionsDonations';
import Link from 'next/link';
import { useFormStatus } from 'react-dom';
import { CreateDonationState } from '@/app/lib/definitionsDonations';
import { useRouter } from 'next/navigation';

export default function CreateDonationForm({ userId }: { userId: number }) {
  const [isRedirecting, setIsRedirecting] = useState(false);
  const [loading, setLoading] = useState(true);
  const [state, formAction] = useActionState<CreateDonationState, FormData>(createDonation, {
    success: false,
    message: '',
    errors: {}
  });
          ...
          <div className="relative mt-2 rounded-md">
            <div className="relative">
              <input
                ...
                type="text"
                placeholder="Nombre de entidad financiera"
                className="peer block w-full rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500"
                required
              />
              <UserIcon className="pointer-events-none absolute left-3 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-gray-500 peer-focus:text-gray-900" />
...
function CreateDonationButton() {
  ...
  return (
    <Button
      ...
      disabled={pending}
    >
      {pending ? 'Registrando...' : 'Registrar datos donacion'} 
