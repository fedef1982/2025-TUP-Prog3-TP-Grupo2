...
import CreateVisitForm from '@/app/ui/visits/create-form';
...
export default async function Page({ params }: { params: { id?: string } }) {
  ...
  try {
    const pubId = Math.floor(Number(resolvedParams.id));

    if (!Number.isSafeInteger(pubId) || pubId <= 0) {
      ...
      <main className="flex min-h-screen flex-col p-6">
        ...
        <div className="mt-4 flex grow flex-col gap-4 md:flex-row">
          <div className="flex flex-col justify-between gap-1 rounded-lg bg-gray-200 px-4 py-5 md:w-1/6 md:px-5">
            <p className={`${lusitana.className} text-xl text-gray-800 md:text-1xl md:leading-normal`}>
              Encontrá tu mascota y coordiná una visita.{' '}
            </p>
            <p className={`${lusitana.className} text-l text-gray-800 md:text-1xl md:leading-normal`}>
          ...
          <div className="w-full">
            <CreateVisitForm publicationId={pubId} />
          </div>
        </div>
      </main>
    );
